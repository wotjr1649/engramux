...
| A Windows CLI flag is mangled into a path — `schtasks /query` becomes `C:/Program Files/Git/query` | MSYS path conversion. Set `MSYS_NO_PATHCONV=1`, or use `//query`. **The two are not interchangeable and the doubled slash is the narrower one.** It works for a subcommand word and not for a single-letter flag: measured 2026-09-06, `taskkill //F //PID <n>` reaches taskkill as `//F` and is refused as an invalid argument, while `MSYS_NO_PATHCONV=1 taskkill /F /PID <n>` kills the process. Prefer the variable |
...
| Two files named from `time.Now()` a millisecond apart get the same name, and the second silently replaces the first | The clock behind a formatted stamp is far coarser than its digits suggest. Measured 2026-09-06: **200,000 samples of `time.Now().UTC().Format(time.RFC3339Nano)` produced 75 distinct strings**, and in a loop shaped like a real writer — a read, a directory scan, a write — **128 of 2,000 consecutive pairs produced the same name**. That is the ~15.6 ms Windows tick, and `internal/host`'s `savedCopies` already names it for *file* times; the name comes off the same clock. `os.WriteFile` on a taken name destroys what is there and returns success, so the caller is handed a name whose contents are not what it wrote. Create with `O_CREATE\|O_EXCL` and disambiguate, which `internal/host.createUntaken` does; a stat first is a race between two of this product's own processes. It cost one intermittent failure that reads as a retention-order bug — `[v5 v4 v2]` for `[v5 v4 v3]` — and reproduced 0 times in 200 runs of that test, because a full `Commit` fsyncs and is slower than a tick |
...
| A splice into a document reports success and inserts nothing, twice in a row | Two silent halves, and they compose. The agent's file-write tool resolves a bare `/tmp/x` against the **current drive** — it lands in `D:\tmp\x` — while this shell's `/tmp` is the MSYS temp, so the two tools disagree about one string with no error from either. And GNU `sed`'s `r` command **ignores a file it cannot read**: no diagnostic, exit 0, nothing inserted. Observed 2026-09-06. The same shape bites a `perl -0pi -e 's/…/…/'` whose literal carries an em dash or anything else re-encoded on the way through the shell: the substitution matches nothing, says nothing, and the document keeps the text you believe you replaced. **Verify the insertion, never the exit code** — `awk` over the lines you meant to change is two seconds and is the only thing that tells an edit from a no-op. It is the document-editing twin of the row below about a mutation that never applied, and it is here separately because that row's advice reads as being about mutation testing |
| You conclude another product cannot read a file you wrote it, and that product's own state says it can | A document shape taken from a vendor's documentation - or from one session's summary of that documentation - is not evidence about the installed binary. Both hosts keep their own record of what they loaded, and it is two commands away. Codex writes `[hooks.state]` into `~/.codex/config.toml`: one entry per hook, keyed by source path, event name and entry index, with a trust hash. Session 17 was handed a finding written as settled - that Codex rejects a top-level `hooks` member and registers nothing - and that table lists all eleven events against that exact file, while the official reference shows the wrapper too. The finding had already reached a backlog row, a work order and a cross-session hand-off before anything checked it. **Read the host's own state before concluding anything about what the host did with your file, and prefer it to the documentation when the two disagree.** It generalises past hooks: what a product records about your input is the only evidence that did not come from you |
| A host's UI prints a hook failure and you take it for yours | Three products register hooks on the same Codex events on this machine, and the failure line names none of them. Backlog 52 was opened against this product's own stdout for a `Stop hook (failed)` that this product cannot produce: the installed Codex trims a `Stop` hook's stdout and an empty result is an explicit do-nothing branch, so the message needs a hook that wrote something. **The event a failure arrives on is not evidence about whose hook it was.** Read the vendor's own handler for what the message requires, then check which of the installed hooks can meet that description - `~/.codex/config.toml`'s `[hooks.state]` lists every source path the host loaded, which is the same table that refuted the previous two diagnoses |
...
| A query is fast in every benchmark and takes four seconds on the real machine | Two things, and the second is the one that generalises. **A window function materialises the whole result set**, so `count(*) OVER ()` sharing a SELECT list with a large column reads that column for every *match* rather than every *hit* - 15,000 payloads to return 20. Select the rowid and the count in an inner query and join the big column onto what survives the LIMIT. `TestGateTheSearchDoesNotReadPayloadsItDoesNotReturn` holds it for the two statements that exist; **a third one written later is not covered**, which is why this is a row. And the reason nobody saw it for months: spec 7.1 measured that shape at **24.2 ms** over 19,503 synthetic events whose documents are a few dozen bytes, where the real ones average about 11.5 KB. **A performance number taken over a corpus that does not resemble the real one can be correct and useless at once** - prefer a ratio between two corpora in one run, where the machine cancels |
