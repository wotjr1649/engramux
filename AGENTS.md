...
| A test runs the installer and quietly edits your own Claude Code configuration | The installer registers the MCP endpoint by running `claude mcp add --scope user`, and that binary resolves its own configuration file — it does not read the `HOME` and `USERPROFILE` the test handed the script, so the redirection that isolates every other file isolates nothing here. What stops it is an empty `PATH`: the script finds `claude` with `where`, `where` is itself resolved through `PATH`, and node is spawned by absolute path and does not need one. `runInstallerWithPath` is the seam; a test that seeds `mcp.json` and applies must use it and must assert the skip line, or it is writing outside its own temporary directory |
