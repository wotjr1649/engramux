...
| # | Where | What |
|---|---|---|
| 38 | `internal/service`'s `TestPhase5GateAReaderDoesNotPushIngestPastItsBudget` | **The Phase 5 contention gate is marginal under `-race` on the owner's machine, and was so before Step 4.** Measured 2026-09-03 at `bd53297`, which carries none of Step 4: five consecutive runs of the gate alone gave a slowest ingest of 692, 784, 852, 777 and 751 ms against spec 5.3's 800 ms - **one of five over**, with the other four inside the budget by less than 50 ms. So a green `scripts/race.sh` on this machine is roughly a four-in-five event rather than a certainty, and a red one is not evidence of a regression until the arm has been measured against a baseline. Step 4 found this by regressing it: `instr(lower(col), ?)` in the boost took the same gate to 832, 845 and 928 ms, none under, and replacing it with `LIKE ... ESCAPE` took it to 375-502 ms across five runs. That fix is committed and is not what this row is about. **What is unowned is the margin itself**, and there are at least three readings and no measurement separating them: the gate's load constants were sized against a machine, `-race` multiplies a cost the budget was written for without it, and `postDialBudget` is spec 5.3's relay budget asserted against the handler alone - which the test's own doc comment calls "a deliberately stricter reading than the clause needs". **Do not fix this by moving the number.** What would settle it is a distribution rather than a run: the same gate, twenty runs, with and without `-race`, on an otherwise idle machine, against which the constants or the reading can be argued. Until then the honest procedure is to re-run a red contention gate once before believing it, and to say which arm was measured |

## Raised by the pre-push review of Step 4, 2026-09-03

| # | Where | What |
|---|---|---|
| 39 | `internal/store`'s `Leaves` and migration `00002`'s backfill | **A lone surrogate escape is decoded differently by the two walks, and has been since `00002` shipped.** Measured 2026-09-03: for a payload carrying `\ud800`, Go substitutes **U+FFFD** where SQLite hands back the **WTF-8 encoding of the code point**, `ED A0 80`. `leaves` is what `events_fts` is built over, so an event ingested live and the same event backfilled by `00002` are indexed differently, and `integrity-check` cannot see it - the index and the column it was built from agree either way. **Go is the side that is right**: an unpaired surrogate is not a character and U+FFFD is what keeps the column valid UTF-8, so "fix Go to match SQLite" would mean writing our own string unescaper and deliberately storing invalid UTF-8. Making SQLite match is not ours to do. So the decision is which wrongness to carry and it is a design decision rather than a repair - which is why this is a row and not a commit. Reachable in practice: Windows permits an unpaired surrogate in a file name, so a tool's output can carry one. Not reachable from either host's own encoder. **A raw invalid UTF-8 byte inside a JSON string is the same defect through a different door** - named by the same review, measured the same day: a raw `0xFF` comes back as U+FFFD from Go and as the byte itself from SQLite. Neither validator rejects it, because Go's scanner and SQLite's `json_valid` both check structure rather than the encoding of string contents, so `sqliteWillParse` cannot remove it either. `TestTheTwoJSONParsersDivergeOnALoneSurrogate` and `TestTheTwoJSONParsersDivergeOnARawInvalidByte` pin the exact bytes on both sides, so a dependency upgrade cannot change this quietly |
| 40 | `internal/store`'s `Derive` and migration `00005`'s backfill | **A duplicated JSON key is resolved differently by the two walks.** Measured 2026-09-03: for `{"command":"first","command":"second"}`, Go's decoder takes the **last** and SQLite's `json_extract` takes the **first**. JSON itself does not say which is right, so neither side is wrong and there is nothing to repair - only a choice about which to adopt, and adopting SQLite's means giving `Derive` a token-stream walk like `Leaves` has rather than a `json.Unmarshal`. Milder than **39** in every direction: the derived columns are a ranking input that nothing selects, so the cost is one pathological document's boost differing between the insert path and the backfill path. `Leaves` is unaffected and the reason is structural - a walk that emits every string leaf visits both values on both sides, where a walk that extracts one member has to choose. Not reachable from either host's encoder, which marshals from maps and structs. `TestTheTwoJSONParsersDivergeOnADuplicatedKey` pins it |
