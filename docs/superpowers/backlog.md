...
| # | Where | What |
|---|---|---|
| 48 | `internal/search`'s ranking, and what a first run returns | **A new user's first search returns the product's own machinery, and on a fresh corpus that is most of the answer.** Measured on the first clean-profile install: six hits for one probe word, of which two carried human text - the prompt and the reply - and four were hook plumbing, including the `PreToolUse` and `PostToolUse` of the MCP search call that was looking for the word. The product found its own act of looking. **Narrowed on 2026-09-05 to the ranking question, which is the half that is still open.** The display half is closed: the excerpt was a rune window centred on the match and aligned to nothing, so it began mid-word - `lUse` was the first observation anyone got of it - and both edges now move to a word boundary within a bounded distance, on its own branch merged `--no-ff`, with backlog 36 beside it because they are the title and the body of the same hit. What is left is whether tool-plumbing events should be weighted below documents carrying prompt or reply text, and that does not get pulled on one observation of six hits: it is a lever on every search every user ever runs, and this repository requires a ranking change to be measured. **The arm is designed and un-run** - memory spec 5's M11 carries the population, the two arms, the closed set, the non-vacuity arm that can close this row without any code, and the delete condition. None of it is repeated here |
