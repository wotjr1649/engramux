...
| # | Where | What |
|---|---|---|
| 51 | `internal/host`'s `CodexEntry` | **A relay path containing a space has no measured spelling, and the quotes just removed are what one would have needed.** The install directory sits under `%LOCALAPPDATA%`, which carries the Windows account name, so an account named with a space reaches it; no account on this machine has one, so nothing here can measure it. What is known, measured 2026-09-05: a `commandWindows` value that is one fully quoted token is echoed rather than run. What is not known is what a path with a space accepts - unquoted breaks on the space, and the quoted form is the spelling just measured broken for a path without one, so **neither of the two obvious answers is available** and a third has to be found and measured rather than reasoned about. Until then Engramux captures nothing from Codex for those users, and `doctor`'s delivery line says so on their machine, which is what it is for. A fix must not be a quote put back without a measurement. `TestCodexTakesThePathWithItsOwnQuotesStrippedOut` is deliberately about the quotes rather than about spaces, so that whatever answers this does not have to argue with a test that pinned the wrong half. Measuring it needs a profile whose account name has a space - the same shape of requirement as the memory spec section 8's clean-profile condition |
| 52 | the relay's stdout, and Codex's `Stop` hook | **Every Codex turn ends with a red line.** `Stop hook (failed) - error: hook returned invalid stop hook JSON output`, observed 2026-09-05 on two consecutive turns of one session. Codex requires a `Stop` hook's stdout to be JSON, spec 4.5 has the relay write nothing on stdout on any event, and empty is not JSON. **It costs no capture, measured**: on 2026-09-05 at 20:40 one Codex session printed this failure twice and two `Stop` events reached the database from it. It costs a user a visible failure on every turn. **It is a design change and not a bug fix**: 4.5 is an invariant, so the question is whether to change it rather than how to make the symptom go away. The narrowest shape is the relay emitting an empty JSON object for Codex `Stop` only, which puts a host-specific branch on the one path that deliberately has none - the relay is told nothing about its host and would have to run spec 4.3's detection itself, in a process that lives about 11 ms, to know which host it is answering. Dropping `Stop` from the 11-event intersection is the other end and costs an event that carries the assistant's reply |
