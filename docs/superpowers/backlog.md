...
| 54 | `cmd/engramux`'s `resolvePaths`, and `CLAUDE_CONFIG_DIR` | **Every path this product derives under `~/.claude` ignores the environment variable that moves it, and the plugin cache added on 2026-09-07 inherits that rather than introducing it.** `internal/memory.ClaudeHome` honours `CLAUDE_CONFIG_DIR`, because M-2's reading found it is the one mechanism that moves a Claude Code configuration home; `resolvePaths` spells its Claude paths as `<home>/.claude/...` with `ENGRAMUX_*` overrides that exist for tests rather than for users. **[verified] 2026-09-07** against the installed host's own strings, which call that variable *"the configuration home"* and name it in a message about *"where your own settings are found"*. On a machine where it is set, `doctor` would report the eleven hook entries missing, `install --apply` would write them where the host will not read them, and the plugin cache line would say there is nothing there. **Nobody has met this**: it is read from the code rather than observed, and nothing measures how many users set that variable. `[unverified]` whether Claude Code's own user-configuration file, which is a sibling of that home rather than inside it, moves with it — a separate question with a separate answer. **The fix is one resolution shared by both, and it is not one path at a time**: a plugin cache honouring the variable beside a `settings.json` that does not is worse than neither. `cmd/engramux` cannot import `internal/memory` to get it — that package reaches `database/sql` and this one is the relay, which `TestTheRelayDoesNotLinkTheSQLiteDriver` exists to keep small — so it wants a leaf both can reach. Product behaviour changes, so it wants its own branch |
