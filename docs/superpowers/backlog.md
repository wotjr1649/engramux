...
| # | Where | What |
|---|---|---|
| 53 | `internal/search`'s ranking, and what the corpus cannot tell it | **A search cannot tell a document that is *about* the query from one that merely *contains* it, and gate M11 is what established that this is the actual problem rather than a weighting one.** Measured over the 901-document corpus: a person's prompt is outside the top ten for its own most distinctive word 64.7% of the time, and every one of those sits under a top ten of pure machinery - so backlog 48's first-run complaint is real and larger than the six hits it was filed on. But the machinery is not there by mistake. The document that ran the command genuinely contains the path, which is why a uniform event-class down-weight fails M11 at every weight in the sweep: it buys the prompt its place by taking the command line's. **What is missing is a signal, not a coefficient**, and naming one is the work: how much of a document the query accounts for, where in it the match falls, whether the match is in text a person wrote or in a field a tool filled. None of those is in the index today - `events_fts` holds string leaves and nothing about their provenance - so this is a schema and indexing question before it is a ranking one, and it is a genuinely open one rather than a deferred fix. Memory spec 5's M11 carries the two arms' figures and the delete condition that closed 48; nothing is repeated here. **Not blocking. All three of the row's named candidates now have a number and three tests own them** - `TestGateM12TheSignalIsWhereTheMatchFell`, `TestGateM13TheQueryShareOfTheDocument` and `TestGateM14TheThresholdM13LeftOpen` in `internal/search`, and the paragraphs above this table say what each one settled. **The row does not close on that.** Nothing ships: `Search` still passes 0 and nil and the ranking is what backlog 48 complained about. **M14 closed the sweep this row was waiting on rather than the row itself.** The threshold half of it is now measured over the full harm populations at nine rungs and there is no clean one at weight 5: at 7,500 ppm four of the five classes are at or above baseline - including 57 of 120 on `a touched path`, the first document any of these candidates has lifted on the class that vetoed the other two - and `an error message` is short of its 79 at every rung in the ladder, best where the gain has already gone. What is left is the **weight**, which M12, M13 and M14 all fixed at 5 and only M11 ever swept, and the candidate nobody has measured: reserving places in the visible list rather than reordering it, which demotes nothing and so has no harm arm of M11's shape at all. Read the harm classes' weight-0 recall before taking that one - 17, 13 and 19 of 25 - because those targets are often outside the top ten already and a reserved slot evicts marginal ones |
| 55 | `internal/inject`'s `queryFor`, and its length tiebreak | **The reduction ranks its non-identifier terms by UTF-8 byte length, which promotes inflected function words over content words — and the effect is not confined to Korean.** Measured 2026-09-08 by calling the real function on synthetic prompts: *이전에 얘기했던 migration 롤백 방법이 뭐였지* reduces to *이전에*, *얘기했던*, *migration* — two conversational fillers and one content word, with *롤백* and *방법* dropped; *세션에서 사용한 golangci-lint 버전 알려줘* keeps *세션에서* and *사용한* and drops *버전*. Hangul costs three bytes a syllable and Latin one, so a byte-length tiebreak systematically prefers the most-inflected Korean token in a mixed prompt over any English one, against a corpus rev.11 measured at 74% English. **English is not exempt**: *how do I fix this build failure again* keeps *again*. The `minTermBytes` comment says it drops *그거*; it does not — that token is six bytes and passes. `TestTheQueryPrefersAnIdentifierToALongerWord`'s first case asserts the reduction picking *마이그레이션이* and *설명해줘* and therefore **records this state as correct**, so changing it is a test change and not only a code change. **Rune length is not the fix and that is the trap**: Korean content words are two syllables and the fillers are four, so the same inversion survives. What the ranking wants is a corpus-frequency signal, and row 53's whole finding is that a frequency signal alone does not separate *about* from *contains* — the rejected rare-anchor arm picked session metadata for exactly that reason. **Deferred rather than fixed**: the owner chose the narrow-injection route on 2026-09-08, so broad selection is not the active goal, and this row is the first thing to read when it becomes one. Nothing measures it over the real population; the discriminating measurement is registered in session 28's brief |
| 56 | `internal/inject`'s query terms, and Korean particle attachment | **A prompt token carries its particle into a prefix query, and the star runs the wrong way for that.** Measured 2026-09-08 over three synthetic documents holding *마이그레이션*, *마이그레이션을* and *마이그레이션이*: the bare stem as a query returns all three, *마이그레이션이* returns one, *마이그레이션을* returns one, and *마이그레이션에서* returns **none**. unicode61 does not split a Hangul stem from its particle, so `"tok"*` matches only tokens beginning with the exact inflection the prompt happened to use — and the injector ANDs three of them. The search package's own doc comment records the direction that works, a bare stem reaching an inflected document, and this is its inverse. **The Latin half was tried and rejected**: `step-query-ascii-particles` carries `queryASCIIStem`, which strips a closed particle set only when the remaining stem is ASCII, so `Codex는` becomes `Codex` and `마이그레이션이` is left alone. **The Hangul-stem case has never been tried and has never been rejected**, and the two must not be confused when this is picked up. Deferred with row 55 and for the same reason |
| 57 | The product's retrieval surfaces, and who is actually reading them | **Engramux has been capturing for its owner and returning nothing to them, and the automatic behaviour the owner believed was this product's belongs to the one it exists to replace.** Established 2026-09-08: injection ships disabled; the owner reports never having called `search`, `get_session_resume` or the CLI once; and the session-start context block the owner had been reading is produced by `thedotmack/claude-mem` 13.15.3, whose `scripts/context-generator.cjs` under the plugin cache is what emits its wording — no string of it appears anywhere in this repository's Go source, and `cmd/engramux/inject.go` records that SessionStart emits nothing by design. **The MCP tools reach the host model as deferred names with no description**, so nothing prompts it to call them; the session that found this had to fetch a schema by hand before it could ask the service anything. So the gap is delivery rather than retrieval quality: M8 P1 puts verbatim recall at 1.000 in all three classes over a corpus nobody queries. **What this buys is an instrument nobody had.** §3 puts task-outcome measurement out of reach for a one-developer corpus, and a live competitor occupying the same slot turns incremental utility into an observable question — the owner's decision to run for some weeks with claude-mem off and Engramux alone is that measurement, and what it produces is experience rather than a labelled score. Not a defect with a fix; a row so that the next reader does not rediscover it |
| 58 | The retrieval surface a person reads, and there is none | **Nothing shows what is held except a count.** `status` answers how many events and `cells` breaks them down by host and event name; there is no way to see what a project holds, what a session contained, or what would be removed by anything. claude-mem ships `ui/viewer.html` with a bundle, a delete button per observation card, a `DELETE /api/observations/:id` endpoint and a `mem-delete` skill - inspected in its installed cache 2026-09-09, so this is what the neighbouring product does rather than what its documentation claims. **Deferred deliberately on 2026-09-09**: the owner's order is to finish and verify the four basic functions first, and a viewer is a new surface this product has never had - an HTTP server handing out raw prompts and file contents, which needs its own authentication and DACL design rather than inheriting the MCP endpoint's. The current-state research names *"user-controlled deletion, and transparent auditing of what information is remembered or forgotten"* as a safe-deployment requirement, so this is closer to a debt than to a convenience for a product that stores what this one stores |
| 59 | The retrieval-side question M15 does not ask | **Every capability class asks for a literal, and the owner's question was not about literals.** P1 is exact-span recall, P3 is temporal, P4 is cross-host, P5 is failure-fix pairs - all of them *"find the string that exists"*. There is no class for a decision and the reasoning behind it, which is what *"what did we decide and why"* needs, and M8 P1's 25 of 25 against native's 4, 1 and 11 was measured on the classes verbatim retrieval is best at. **So M-1 is not refuted by what has run; the thing the owner pointed at was never tested.** What a measurement would need is a class whose questions are not cut from documents - and the hard part is that whoever knows the answers must not also write the questions, or it measures the author rather than the corpus. The material for it already exists unindexed-by-nobody: both hosts' own consolidated artefacts are in `memory_items` and a search already returns them as `memory_hits`, so the first arm is whether *those* answer such questions before anything of our own gets built. **The deferral has lapsed and the priority has not.** It was deferred behind M15 by the owner's choice of the storage-side question on 2026-09-09; M15 ran the same day and answered **0.00%**, so nothing is waiting on it any more. What M15 added to this row is an argument rather than a schedule: its criterion - droppable means unreachable by a query cut from the document's own text - could only ever approach zero, because `a path basename` derives a candidate from all 46,806 events and reaches all of them, and a query cut out of a document finds it by construction. So the gates cannot separate a valuable event from a worthless one, which is this row's claim arriving from the storage side. **The owner's order of 2026-09-09 still puts this fifth**, behind Phase 2's execution design, the deletion and inventory CLI, and the long-term selection rules |
| 60 | Nothing reports how large the database is | **The one indicator that moves before anything degrades is the one nobody is shown.** Neither `doctor` nor `status` prints a size - `status` prints the database's *path* - and no threshold of any kind is in the tree: `grep -rn "2147483648\|2 GB" internal/ cmd/` finds nothing, and the only `Size()` calls in `cmd/engramux/doctor.go` read the log tail. Found 2026-09-09 while correcting the memory spec's compaction paragraph, which had asserted for weeks that `doctor` already reported it. **The trigger it was supposed to feed is gone rather than broken**: the owner took two gigabytes off the promise the same day, so this is no longer a condition nobody can evaluate - it is a number nobody can see, and what it would inform is when to prioritise the deletion and inventory CLI. **Not blocking, and the urgency is measured rather than assumed.** Disk is not the constraint - 734 GB free at 512 MB - and the harm the size proxies for is the cold read, whose signal already ships: `status` prints `errors`, the read-deadline failures are exactly those records, and they ran 4, 2, 6 and 9 per day through 09-04 and then **0 across 09-05 to 09-09** while the file went 180 → 512 MB, so migration `00003`'s covering index held through a near-tripling. That signal has two limits worth knowing before leaning on it: it is a counter since service start, so a restart clears it and only the log survives, and it is a cliff rather than a slope - a read that is slower and still succeeds raises nothing. **The work is small and the mechanism is already written twice.** `scripts/soak-sample.sh` reads both files from outside the service with `stat -c %s`, which is the whole trick given I-07, and `cmd/engramux/doctor.go`'s local section already resolves the data directory it would stat. Adding a line to `status` does not break the soak sampler either, which parses by field name with `awk`. Deliberately not done on 2026-09-09: the owner's order puts Phase 2 next, and this is a candidate to fold into the deletion and inventory CLI rather than a detour before it |
