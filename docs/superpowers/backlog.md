...
| 29 | `EventDocument.ID` and `SearchHit.ID` | **`events.id` reaches an MCP reader neither masked nor bounded**, where the session id beside it is masked and the event name is masked and then bounded (spec 6.1). Raised by the Phase 6 review, on code that predates it. The column takes whatever the envelope's `IngestID` said and `pipe.validate` checks only that it is non-empty, so the bound on the way *out* is nothing at all - `MaxEventIDBytes` bounds a `get_event` request, not a reply. What makes this a row and not a defect is provenance: an event name is whatever a host's payload said, and an id is minted by the relay as a UUIDv7, so a secret-shaped id needs one of this user's own processes to write one - a hand-edited or corrupted spool record is the only route. Spec 6.1 records it as the second known perimeter and spec 8's Phase 6 row says why the audit does not load it. Fixing it is `secret.MaskString` on both fields, which is the answer `list_sessions` already gives for an id it must not truncate - a change to a shipped binary, which is why it did not happen inside a running soak |
