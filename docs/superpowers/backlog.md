...
| # | Where | What |
|---|---|---|
| 42 | `cmd/engramux`'s `doctor.go` and `inject.go`, and 1.0 spec §7.1 | **The relay binary links the SQLite driver, against an invariant this repository declared and still repeats.** Measured 2026-09-04: `go list -deps ./cmd/engramux` reports `modernc.org/sqlite` and `github.com/pressly/goose/v3`, reached through `internal/inject` → `internal/search` → `internal/store`, which both `doctor.go` and `inject.go` import. `dist/engramux.exe` is **8,703,488 B** against the **3,862,528 B** §7.1 records twice. The session-05 brief states the rule outright - *"The relay binary must not link the SQLite driver"* - and `doctor.go`'s own comment explains that it uses `spool.Dir()` rather than importing `internal/store` *because* that would link the driver, in the file that imports it anyway two hundred lines above. What this costs is not only bytes: §5.9's rejection of an stdio proxy and `doctor`'s rejection of a `net/http` probe (+93.7%) are both argued against the 3,862,528 B baseline, so both rejections now rest on a figure that has moved. The relay is spawned once per hook event, which is what made the size an invariant rather than a preference. **Not a one-line fix**: it is a decision about how `doctor` reports injection and how the `inject` CLI verb is reached without the reader half being in the relay's graph. No test owns it, and one could - a dependency assertion over `go list -deps` would fail the moment it happens again |
| 43 | `internal/mcpconf`'s `Write` | **The temporary file that holds a raw bearer token has no sweep, and the one that does not need one has.** `internal/host.writeAtomic` calls `staleTemps` before it writes, which removes whatever an earlier run left when it was killed between the write and the rename; `mcpconf.Write` has no equivalent, and it is `mcpconf`'s temporary file that carries the token verbatim rather than inside another product's configuration. `os.CreateTemp`'s random suffix means every killed run leaves its own file under a name nothing will ever replace, and the token is sticky across restarts, so the copy stays valid. `TestWriteLeavesNoTemporaryFile` covers the success path only. The asymmetry was noticed while closing 28 and is deliberately not fixed in that change: it is a different failure mode from the DACL and wants its own test. Note that after 28 the leftover is at least narrowed, so the exposure is smaller than it was |
| 44 | `internal/host`'s `backup` | **Every `Commit` leaves a timestamped copy of the file it replaced, and nothing sweeps them or bounds how many there are.** `backupInfix` is referenced in one place, there is no retention policy, and `doctor` does not count them. The file this matters for is `~/.codex/config.toml`, which holds `Authorization = "Bearer <token>"` - so a backup taken after the token was written is another long-lived copy of it, under a name nothing replaces, in a directory people are asked to attach to bug reports. `write.go`'s own comment already states the hazard. **The count on the owner's machine is `[unverified]`**: a credential-directory guard refused the listing and it was not worked around. What a fix looks like is either a sweep on the pattern of `staleTemps` or a `doctor` line counting them; which is right depends on whether a backup is meant to be recoverable |
