...
| 50 | `cmd/engramux`'s `doctor`, and what a Codex hook actually does | **No Codex event has ever been captured through a hook Engramux wrote, and `doctor` has reported that host healthy the whole time.** Measured 2026-09-05 from the owner's own database, per host and event: `claude-code` has all eleven event names and 29,176 events, the newest arriving as the query ran. `codex` has two names - 3 `SessionEnd`, last on 2026-08-28 21:28, and 54 `SessionStart` of which all but a few are backlog 49's phantoms. There is no `codex UserPromptSubmit` at all, in a database holding 468 Claude Code ones, against **83 Codex session rollouts since 2026-08-29**. The raw corpus's 118 Codex captures are not counter-evidence: every one of them was received between 2026-08-27 02:30 and 04:44, **before** Engramux's first write to that file at 2026-08-28 17:31, and their `_cap.argv` records a capture probe invoked as `--host codex --event <name>` - a different binary and a different command line. **The first diagnosis of this row was wrong and is kept here because of how it was reached.** It said `MergeHooks` writes the eleven event names under a top-level `hooks` member that Codex does not recognise. Three things refute it. The official reference shows that file **with** a top-level `hooks` member (`{"description": ..., "hooks": {"SessionStart": [...]}}`). Codex's own `~/.codex/config.toml` carries a `[hooks.state]` table keying **all eleven events** to `~/.codex/hooks.json`, one `trusted_hash` each, event names normalised to snake_case and entries identified by index - so Codex opens that file, reads it through the wrapper, and identifies every entry in it. And the wrapper predates this product: the 454-byte file Engramux first backed up on 2026-08-28 already had it, holding someone else's `PreToolUse` hook. The owner then checked `/hooks` in the CLI: **all eleven are trusted**, so the documented trust gate is not it either. `MergeHooks` is correct as written and needs no per-host root; nothing was malformed, so the question of what to do about a malformed file on an installed machine does not arise. **What is still open is why the hooks do not fire**, and three candidates are `[unverified]`, in the order the evidence supports them. **Enabled is not trusted**: the `/hooks` browser can disable an individual non-managed hook, which is a different axis from the trust hash, and nobody has looked at it - cheapest to check and it would account for all eleven at once. **The command spelling**: `CodexEntry` writes the program token quoted inside the value and passes no arguments, where every hook that demonstrably runs on this machine spells an unquoted program token followed by quoted arguments. Against it, a fully quoted absolute path is a valid Windows command line, so this is a difference rather than a defect until something measures it. **The matcher**: `"*"` is documented as matching everything, but openai/codex#22847 reports `"*"` matchers misbehaving - it cannot be the whole story, because the six events Engramux gives a matcher to are disjoint from the five it does not and all eleven are silent. Nothing this repository can read discriminates them: `~/.codex/logs_2.sqlite` records `hook/started` and `hook/completed` as app-server notifications carrying no hook identity, and the session rollouts carry no hook records at all. It takes a Codex session with somebody watching, because a hook failure is reported in the TUI. **The part of this row that survives the refutation is the reason it was filed.** `readOneHostHooks` answers `codex 11 of 11 events point at the installed relay` by reading the file through the member name the installer wrote it under: this product validating its own output against its own assumption, which is backlog 49's failure mode one layer out. That sentence was true and useless for nine days. A static check of a file this product produced cannot see a host that never runs it. **The fix is a `doctor` line reporting the last event actually received per host**, which the service already computes for `cells`, and which would have caught this on day one rather than the ninth. **Built and merged 2026-09-05** (`backlog-50-doctor-reports-what-arrived`, `--no-ff`), and measured on the owner's own machine straight after: the two hosts' configuration lines are the same sentence and their delivery lines are `claude-code received 29855 events, the last at 2026-09-05 16:54:57` against `codex received 3 events, the last at 2026-08-28 21:28:39`. A service too old to send the breakdown answers `unknown` rather than zero, which was checked against the installed service before the reinstall. The wrong diagnosis is itself an instance of the same pattern: it was taken from one session's reading of the documentation rather than from the host's own state, and the state that refutes it was two commands away |
