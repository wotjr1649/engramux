...
func gateClass(t *testing.T, db *sql.DB, mode string, c class, docs []doc) {
	...
	for _, cd := range cands {
		hits, _, err := search.Search(t.Context(), db, cd.query, "", len(docs), search.MatchAll)
		if err != nil {
...
func escapesTheIntersection(
	...
	for i, term := range terms {
		...
		alone, _, err := search.Search(t.Context(), db, term, "", limit, search.MatchAll)
		if err != nil {
...
func gatePrecision(t *testing.T, db *sql.DB, mode string, docs []doc) {
	...
	hits, _, err := search.Search(t.Context(), db, precisionKey, "", len(docs), search.MatchAll)
	if err != nil {
